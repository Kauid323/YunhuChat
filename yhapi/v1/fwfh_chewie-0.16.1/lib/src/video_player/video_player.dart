...
class _VideoPlayerState extends State<VideoPlayer> {
  ...
  static const String _jwznbReferer = 'https://myapp.jwznb.com';

  bool _needReferer(String url) {
  ...
  Future<void> _initControllers() async {
    final headers = _needReferer(widget.url) ? const {'Referer': _jwznbReferer} : const <String, String>{};
    debugPrint('[fwfh_chewie][video] init url=${widget.url} headers=$headers');
    ...
    try {
      ...
      debugPrint(
        '[fwfh_chewie][video] initialized url=${widget.url} duration=${vpc.value.duration} size=${vpc.value.size} aspect=${vpc.value.aspectRatio}',
      );
    } catch (error) {
      vpcError = error;
      debugPrint('[fwfh_chewie][video] initialize FAILED url=${widget.url} error=$error');
    }
